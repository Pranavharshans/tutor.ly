...
const VideosPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const llmResponse = await fetch("/api/videos", {
        method: "POST",
      ...
      if (!llmResponse.ok) {
        throw new Error(`LLM API failed: ${llmResponse.status} ${llmResponse.statusText}`);
      }
      
      const llmData = await llmResponse.json();
      console.log("LLM Response:", llmData);
      
      if (llmData.manimScript) {
        console.log("Generated Manim script:\n", llmData.manimScript);
        
        // 2. Send the script to the local FastAPI worker (Fixed port to 8000)
        console.log("Sending script to local FastAPI server...");
        const renderResp = await fetch("http://127.0.0.1:8000/render", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ script: llmData.manimScript })
        });
        
        console.log("Render response status:", renderResp.status);
        
        if (!renderResp.ok) {
          const errorText = await renderResp.text();
          console.error("Render failed:", errorText);
          throw new Error(`Render failed: ${renderResp.status} - ${errorText}`);
        }
        ...
        if (renderData.videoUrl) {
          ...
          setError("No video URL returned from render service");
        }
      } else if (llmData.error) {
        setError(`LLM error: ${llmData.error}`);
      } else {
        setError("No script generated by LLM");
      }
    } catch (err) {
